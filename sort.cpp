...
void printFile(std::string fileName) {
    ...
    while (remainingInts > 0) {
        ...
        out.read((char*)chunk.data(), thisChunksSize * sizeof(int));
        for (int j = 0; j < thisChunksSize; j++)
            std::cout << chunk[j] << " ";
...
void mergeSortedChunks(std::fstream& temp, std::ostream& out, std::vector<chunkInfo>& chunkInfoArr, int numChunks) {
    int numInts;
    temp.seekg(0);
    ...
    while (!minheap.empty()) {
        ...
        if (buffers[minIndex].empty() and chunkInfoArr[minIndex].numsLeft > 0)
            replenishBuffer(buffers, chunkInfoArr, temp, subchunkSize, minIndex);

        // 3.5. If the buffer is not empty, push its minheap value (i.e first) to the minheap
